...
namespace TrainEngine.Core.Web.HttpHandlers
{
	...
	public class JavaScriptHandler : IHttpHandler
	{
			...
			if (!string.IsNullOrEmpty(path))
			{
				...
				if (!string.IsNullOrEmpty(script))
				{
					...
					if (TrainSettings.Instance.EnableHttpCompression)
						TrainEngine.Core.Web.HttpModules.CompressionModule.CompressResponse(context);//Compress(context);
				}
			}
		}
		...
		{
			string[] lookfor = ConfigurationManager.AppSettings.Get("TrainEngine.HardMinify").Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
		...
		/// </summary>
		private static string StripWhitespace(string body, bool isTrainEngineScript)
		{
			...
			if (isTrainEngineScript)
			{
				...
				body = Regex.Replace(body, "\\r\\n(?<=\\b(abstract|boolean|break|byte|case|catch|char|class|const|continue|default|delete|do|double|else|extends|false|final|finally|float|for|function|goto|if|implements|import|in|instanceof|int|interface|long|native|new|null|package|private|protected|public|return|short|static|super|switch|synchronized|this|throw|throws|Trainsient|true|try|typeof|var|void|while|with)\\r\\n)", " ", RegexOptions.Compiled | RegexOptions.ECMAScript);
				// remove all newline
				//body = Regex.Replace(body, "\\r\\n", "", RegexOptions.Compiled | RegexOptions.ECMAScript);
