	...
	public class SyndicationHandler : IHttpHandler
	{
		...
		private static List<IPublishable> GenerateItemList(HttpContext context)
		{
			if (!string.IsNullOrEmpty(context.Request.QueryString["category"]))
			{
                ...
                return Training.GetTrainingsByCategory(categoryId).ConvertAll(new Converter<Training, IPublishable>(ConvertToIPublishable));
			}
			...
			if (!string.IsNullOrEmpty(context.Request.QueryString["author"]))
			{
                ...
                return Training.GetTrainingsByAuthor(author).ConvertAll(new Converter<Training, IPublishable>(ConvertToIPublishable));
			}

			if (!string.IsNullOrEmpty(context.Request.QueryString["post"]))
			{
				// All comments of the specified post
                Training post = Training.GetTraining(new Guid(context.Request.QueryString["training"]));
            ...
            return Training.Trainings.ConvertAll(new Converter<Training, IPublishable>(ConvertToIPublishable));
        }
		...
		private static List<IPublishable> RecentComments()
		{
            ...
            foreach (Training post in Training.Trainings)
			{
				foreach (Comment comment in post.DisposedComments)
				{
					temp.Add(comment);
				}
			}
		...
		private static SyndicationFormat RetrieveFormat(HttpContext context)
		{
			...
			string format = TrainSettings.Instance.SyndicationFormat;
		...
		{
			string title = TrainSettings.Instance.Name;
			...
			if (!string.IsNullOrEmpty(context.Request.QueryString["training"]))
			{
                if (context.Request.QueryString["training"].Length != 36)
					StopServing(context);

                Training training = Training.GetTraining(new Guid(context.Request.QueryString["training"]));
                if (training == null)
				{
					StopServing(context);
				}
		...
		private static void SetHeaderInformation(HttpContext context, List<IPublishable> items, SyndicationFormat format)
		{
			...
			{
				if (item.DateModified.AddHours(-TrainSettings.Instance.Timezone) > lastModified)
					lastModified = item.DateModified.AddHours(-TrainSettings.Instance.Timezone);
			}
