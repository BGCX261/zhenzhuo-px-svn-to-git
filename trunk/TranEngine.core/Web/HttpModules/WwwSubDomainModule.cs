...
namespace TrainEngine.Core.Web.HttpModules
{
  ...
  public class WwwSubDomainModule : IHttpModule
  {
    ...
    {
      if (TrainSettings.Instance.HandleWwwSubdomain == "ignore" || string.IsNullOrEmpty(TrainSettings.Instance.HandleWwwSubdomain))
        return;
      ...
      if (context.Request.PhysicalPath.EndsWith(".aspx", StringComparison.OrdinalIgnoreCase))
      {
        ...
        if (url.Contains("://www.") && TrainSettings.Instance.HandleWwwSubdomain == "remove")
          RemoveWww(context);
        
        if (!url.Contains("://www.") && TrainSettings.Instance.HandleWwwSubdomain == "add")
          AddWww(context);        
      }
    }
