...
namespace TrainEngine.Core.Web.HttpModules
{
	/// <summary>
	...
	public class UrlRewrite : IHttpModule
	{
		...
		private void context_BeginRequest(object sender, EventArgs e)
		{
			...
			if (url.Contains(TrainSettings.Instance.FileExtension.ToUpperInvariant()) && !url.Contains("ERROR404.ASPX"))
			{
				

				if (url.Contains("/TRAINING/"))
				{
					RewriteTraining(context, url);
				}
				else if (url.Contains("/CATEGORY/"))
				{
					RewriteCategory(context, url);
				}
                else if (url.Contains("/CURRICULA/"))
				{
                    RewriteCurricula(context, url);
				}
				else if (url.Contains("/CALENDAR/"))
				{
					context.RewritePath(Utils.RelativeWebRoot + "default.aspx?calendar=show", false);
				}
				else if (url.Contains("/DEFAULT" + TrainSettings.Instance.FileExtension.ToUpperInvariant()))
				{
				...
				else if (url.Contains("/AUTHOR/"))
				{
					...
					context.RewritePath(Utils.RelativeWebRoot + "default" + TrainSettings.Instance.FileExtension + "?name=" + author + GetQueryString(context), false);
				}
        ...
        private static void RewriteTraining(HttpContext context, string url)
		{
			DateTime date = ExtractDate(context);
			string slug = ExtractTitle(context, url);
            Training train = Training.Trainings.Find(delegate(Training p)
			{
				...
				return slug.Equals(Utils.RemoveIllegalCharacters(p.Title), StringComparison.OrdinalIgnoreCase);
			});
            ...
            if (train != null)
			{
				...
				{
                    context.RewritePath("syndication.axd?post=" + train.Id.ToString() + GetQueryString(context), false);
				}
				else
				{
                    context.RewritePath(Utils.RelativeWebRoot + "training.aspx?id=" + train.Id.ToString() + GetQueryString(context), false);
				}
			}
		}
		...
		private static string ExtractTitle(HttpContext context, string url)
		{
			...
			if (url.Contains(TrainSettings.Instance.FileExtension) && url.EndsWith("/"))
			{
			...
			url = url.Substring(0, url.IndexOf(TrainSettings.Instance.FileExtension));
