...
namespace TrainEngine.Core
{
	...
	public class SyndicationGenerator
	{
		...
		/// </summary>
		private const string GENERATOR_NAME = "TrainEngine.Net Syndication Generator";
		/// <summary>
		...
		/// <summary>
		/// Private member to hold the <see cref="TrainSettings"/> to use when generating syndication results.
		/// </summary>
		private TrainSettings blogSettings;
		/// <summary>
		...
		#region Settings
		/// <summary>
		/// Gets or sets the <see cref="TrainSettings"/> used when generating syndication results.
		/// </summary>
		/// <value>The <see cref="TrainSettings"/> used when generating syndication results.</value>
		...
		{
			int offset = (int)(TimeZone.CurrentTimeZone.GetUtcOffset(DateTime.Now).TotalHours + TrainSettings.Instance.Timezone);
		...
		private void WriteRssChannelCommonElements(XmlWriter writer)
		{
			...
			writer.WriteElementString("generator", "TrainEngine.NET " + TrainSettings.Instance.Version());
