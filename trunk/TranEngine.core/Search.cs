...
namespace TrainEngine.Core
{
	...
	public static class Search
	{
		...
		{
			BuildCatalog();
            Training.Saved += new EventHandler<SavedEventArgs>(Training_Saved);
            Curricula.Saved += new EventHandler<SavedEventArgs>(Curricula_Saved);
			TrainSettings.Changed += delegate { BuildCatalog(); };
            Training.CommentAdded += new EventHandler<EventArgs>(Training_CommentAdded);
            Training.CommentRemoved += delegate { BuildCatalog(); };
            Comment.Approved += new EventHandler<EventArgs>(Training_CommentAdded);
		}
		...
		/// </summary>
        private static void Training_Saved(object sender, SavedEventArgs e)
		{
			lock (_SyncRoot)
			{
				if (e.Action == SaveAction.Insert)
				{
					AddItem(sender as Training);
				}
				else
				{
					BuildCatalog();
        ...
        private static void Curricula_Saved(object sender, SavedEventArgs e)
		{
			lock (_SyncRoot)
			{
				if (e.Action == SaveAction.Insert)
				{
                    AddItem(sender as Curricula);
				}
				else
				{
					BuildCatalog();
        ...
        static void Training_CommentAdded(object sender, EventArgs e)
		{
		...
		private readonly static StringCollection _StopWords = TrainEngine.Core.Providers.TrainService.LoadStopWords();
