...
namespace TrainEngine.Core
{
	...
	public static class Utils
	{
		...
		{
            return Utils.StringIsNullOrWhitespace(html) ? string.Empty : STRIP_HTML.Replace(html, string.Empty).Trim();
		...
		public static string FeedUrl
		{
			get
			{
				if (!string.IsNullOrEmpty(TrainSettings.Instance.AlternateFeedUrl))
					return TrainSettings.Instance.AlternateFeedUrl;
				else
		...
		public static string RelativeWebRoot
		{
			get
			{
				if (_RelativeWebRoot == null)
					_RelativeWebRoot = VirtualPathUtility.ToAbsolute(ConfigurationManager.AppSettings["TrainEngine.VirtualPath"]);
        ...
        /// <summary>
        /// Trainslates the specified string using the resource files.
        /// </summary>
        ...
        /// <summary>
        /// Trainslates the specified string using the resource files.  If a Trainslation
        ...
        /// <summary>
        /// Trainslates the specified string using the resource files and specified culture.
        /// If a Trainslation is not found, defaultValue will be returned.
        /// </summary>
        ...
        {
            if (string.IsNullOrEmpty(TrainSettings.Instance.Culture) ||
                TrainSettings.Instance.Culture.Equals("Auto", StringComparison.OrdinalIgnoreCase))
            {
                return CultureInfo.InstalledUICulture;
            }
            return CultureInfo.CreateSpecificCulture(TrainSettings.Instance.Culture);
        }
		...
		private static readonly Regex MOBILE_REGEX = new Regex(ConfigurationManager.AppSettings.Get("TrainEngine.MobileDevices"), RegexOptions.IgnoreCase | RegexOptions.Compiled);
		...
		public static void SendMailMessage(MailMessage message)
		{
			...
			try
			{
				...
				SmtpClient smtp = new SmtpClient(TrainSettings.Instance.SmtpServer);
                // don't send credentials if a server doesn't require it,
                // linux smtp servers don't like that 
                if (!string.IsNullOrEmpty(TrainSettings.Instance.SmtpUserName)) {
				    smtp.Credentials = new System.Net.NetworkCredential(TrainSettings.Instance.SmtpUserName, TrainSettings.Instance.SmtpPassword);
                }
				smtp.Port = TrainSettings.Instance.SmtpServerPort;
				smtp.EnableSsl = TrainSettings.Instance.EnableSsl;
				smtp.Send(message);
				OnEmailSent(message);
			}
			catch (SmtpException)
        ...
        /// attributed set to "TrainEngineExtension" it will
            ...
            foreach (string fileName in fileEntries)
            {
                if (fileName.EndsWith(".dll", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (attr != null && attr.Length > 0)
                    {
                        ...
                        if (aca != null && aca.Configuration == "TrainEngineExtension")
                        {
                            assemblies.Add(asm);
                        }
                    }
