<%@ Page Language="C#" AutoEventWireup="true" CodeFile="regorg.aspx.cs" Inherits="reg_regorg" Title="培训机构注册"%>
<%@ Import Namespace="TrainEngine.Core" %>

<asp:content id="Content1" contentplaceholderid="cphBody" runat="Server">
<div class="Div_Position" id="DivPosition"  > <span style="padding-left: 12px;font-size:14px;">培训机构注册</span> </div>
<div class="divRegBox">
<style>
...
<script>
    ...
    function fnt_on(target_name) {
        ...
        switch (target_name) {
            ...
            case "td_email": { my_target.innerHTML += "正常接收邮件的邮箱,便于客户通过邮件咨询或报名"; } break;
            case "td_xingming": { my_target.innerHTML += "请输入您的姓名"; } break;
            ...
            case "td_fax": { my_target.innerHTML += "请正确填写,如(020-12345678)"; } break;
            case "td_qqmsn": { my_target.innerHTML += "请正确填写以便我们及时联系(例如:qq:123456 msn:abc@abc.com)"; } break;
            case "td_company": { my_target.innerHTML += "请填写在工商部门注册的公司名称"; } break;
            case "td_diqu": { my_target.innerHTML += "请选择"; } break;
            case "td_address": { my_target.innerHTML += "建议填写目前办公地点,体现公司的真实性"; } break;
            case "spn_jianjie": { my_target.innerHTML += "请填写,字数至少50个"; } break;
            case "spn_kehu": { my_target.innerHTML += "请填写服务过的客户,如无留空"; } break;
            case "td_home": { my_target.innerHTML += "请填写公司网址"; } break;
            case "td_logo": { my_target.innerHTML += "请上传公司logo(规格:130*50 图片格式:gif,jpg)"; } break;
    ...
    function fnt_uid(ajax) {
        ...
        if (reg_uid.value == "") { td_uid.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>必填</font>"; return "请填写用户名"; }
            ...
            else { td_uid.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>错误 字母,数字 4～20字符</font>"; return "用户名错误 字母,数字 6～20字符"; }
        }
    }
    function ajax_uid() {
        var reg_uid = document.getElementById("reg_uid");
    ...
    function fnt_pwd1() {
        ...
        if (reg_pwd1.value == "") { td_pwd1.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>必填</font>"; return "请填写密码"; }
            ...
            else { td_pwd1.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>错误 字母,数字 6～20字符</font>"; return "密码错误 字母,数字 6～20字符"; }
        }
    }
    //确认密码
    function fnt_pwd2() {
    ...
    function fnt_xingming() {
        ...
        if (reg_xingming.value == "") { td_xingming.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>必填</font>"; return "请填写联系人"; }
    ...
    function fnt_fax() {
        var message = "传真";
        var reg_fax1 = document.getElementById("reg_fax1");
        var reg_fax2 = document.getElementById("reg_fax2");
        var td_fax = document.getElementById("td_fax");
        var rg_fax1 = /^\d{3,4}$/;
        var rg_fax2 = /^\d{7,8}$/;

        if (reg_fax1.value != "" & rg_fax1.test(reg_fax1.value) == false) { message += ",区号错误(应是3～4位数字)"; }

        if ((reg_fax1.value != "" | reg_fax2.value != "") & rg_fax2.test(reg_fax2.value) == false) { message += ",电话号错误(应是7～8位数字)"; }

        if (message == "传真") { td_fax.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/ok.gif' />"; return ""; }
        else { td_fax.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>" + message + "</font>"; return message; }
    }
    //qqmsn
    function fnt_qqmsn() {
    ...
    function fnt_company() {
        ...
        if (reg_company.value == "") { td_company.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>必填</font>"; return "请填公司全称"; }
        else { td_company.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/ok.gif' />"; return ""; }
    }
    //省市
    ...
    //地址
    function fnt_address() {
        var td_address = document.getElementById("td_address");
        td_address.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/ok.gif' />";
        return "";
    }
    //公司简介
    ...
    //logo
    function fnt_logo() {
        var file_logo = document.getElementById("<%=fudPhoto.ClientID %>");
        var td_logo = document.getElementById("td_logo");
        var li_name = file_logo.value.split(".");
        if (file_logo.value == "" | li_name[li_name.length - 1].toLowerCase() == "gif" | li_name[li_name.length - 1].toLowerCase() == "jpg") { td_logo.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/ok.gif' />"; return ""; }
        else { td_logo.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>图片格式不正确</font>"; return "图片格式不正确"; }
    }
    ...
    function fnt_sbm(oEvent) {
        ...
        if (err_message == "") { err_message = fnt_fax(); err_target = "reg_fax1"; } else { fnt_fax(); }
        ...
        if (err_message == "") { err_message = fnt_address(); err_target = "reg_address"; } else { fnt_address(); }
        ...
        if (err_message == "") { err_message = fnt_logo(); err_target = "file_logo"; } else { fnt_logo(); }
...
<tr class="head"><td colspan="3" style="border:none;background:url(/img/reg/btn5.jpg) left bottom no-repeat;">帐号基本信息</td></tr>
...
<tr><td class="td1"><span>*</span> 电子邮箱：</td><td><input name="reg_email" id="reg_email" type="text" maxlength="100" onblur="fnt_email(1)" onfocus="fnt_on('td_email')" /></td><td id="td_email" class="td3">正常接收邮件的邮箱,便于客户通过邮件咨询或报名</td></tr>
...
<tr class="head"><td colspan="3" style="background:url(/img/reg/btn4.jpg) left bottom no-repeat;">联系人信息</td></tr>

<tr><td class="td1"><span>*</span> 联系人：</td><td><input name="reg_xingming" id="reg_xingming" type="text" maxlength="50" onblur="fnt_xingming()" onfocus="fnt_on('td_xingming')" /></td><td id="td_xingming" class="td3">请输入您的姓名</td></tr>
...
<tr><td class="td1">传真号码：</td><td><input name="reg_fax1" id="reg_fax1" type="text" maxlength="4" onblur="fnt_fax()" onfocus="fnt_on('td_fax')" /> - <input name="reg_fax2" id="reg_fax2" type="text" maxlength="8" onblur="fnt_fax()" onfocus="fnt_on('td_fax')" /></td><td id="td_fax" class="td3">请正确填写,如(020-12345678)</td></tr>
...
<tr class="head"><td colspan="3" style="background:url(/img/reg/btn6.jpg) left bottom no-repeat;">公司详细信息</td></tr>

<tr><td class="td1"><span>*</span> 公司全称：</td><td><input name="reg_company" id="reg_company" type="text" maxlength="100" onblur="fnt_company()" onfocus="fnt_on('td_company')" /></td><td id="td_company" class="td3">请填写在工商部门注册的公司名称</td></tr>
...
<tr><td class="td1">公司地址：</td><td><input name="reg_address" id="reg_address" type="text" maxlength="200" onfocus="fnt_on('td_address')" onblur="fnt_address()" /></td><td id="td_address" class="td3">建议填写目前办公地点,体现公司的真实性</td></tr>

<tr><td class="td1"><span>*</span> 公司简介：</td><td colspan="2"><textarea name="reg_jianjie" id="reg_jianjie" onkeyup="fnt_jianjie_count()" onfocus="fnt_on('spn_jianjie')" onblur="fnt_jianjie()"></textarea><span id="jianjie_count"></span><span id="spn_jianjie">请填写,字数至少50个</span></td></tr>
...
<tr><td class="td1">公司logo：</td><td><asp:FileUpload ID="fudPhoto" runat="server" /></td><td id="td_logo" class="td3">请上传公司logo(规格:130*50 图片格式:gif,jpg)</td></tr>
