<%@ Page Language="C#" AutoEventWireup="true" CodeFile="regtch.aspx.cs" Inherits="reg_regtch" Title="培训讲师注册"%>
<%@ Import Namespace="TrainEngine.Core" %>

<asp:content id="Content1" contentplaceholderid="cphBody" runat="Server">

<div class="Div_Position" id="DivPosition"  > <span style="padding-left: 12px;font-size:14px;">培训讲师注册</span> </div>
<div class="divRegBox">
<style>
input,select,option{font:12px/14px Arial; vertical-align: middle;}
.pd { margin-left:12px;margin-top:-21px;}	

.div1{margin-top:10px;border:solid 1px #ccc;padding:20px 100px;}
.tbl1{width:100%;}
...
#reg_xingming{width:230px}
#reg_pay{width:154px;}
.spn_pay{vertical-align:middle;color:#666;}
#<%=clientID%>{width:406px;display:inline-block;vertical-align:middle;_display:inline;}
#<%=clientID%> td{width:1%;padding:2px 0px;}
#spn_lingyu{color:#666;margin-left:10px;}
#spn_lingyu img{vertical-align:middle;}

#reg_jianjie{width:400px;height:80px;vertical-align:middle;}
#spn_jianjie{vertical-align:middle;color:#666;}
#spn_jianjie img{vertical-align:middle;}
#jianjie_count{margin-left:10px;}
#reg_kehu{width:400px;height:80px;vertical-align:middle;}
#spn_kehu{vertical-align:middle;margin-left:10px;color:#666;}
#spn_kehu img{vertical-align:middle;}


#reg_zhujiang{height:60px;width:400px;vertical-align:middle;}
#spn_zhujiang{vertical-align:middle;margin-left:10px;color:#666;}
#spn_zhujiang img{vertical-align:middle;}


#reg_home{width:230px}
#reg_phone1{width:30px;}
#reg_phone2{width:92px;}
#reg_phone3{width:60px;}
#txt1{width:500px;height:80px;}
#reg_sex_1{margin-left:20px;}
...
<script>
    ...
    function fnt_on(target_name) {
        ...
        switch (target_name) {
            ...
            case "td_lianxiren": { my_target.innerHTML += "请输入联系人姓名"; } break;
            ...
            case "td_xingming": { my_target.innerHTML += "讲师姓名"; } break;
            case "td_diqu": { my_target.innerHTML += "请选择"; } break;
            case "td_pay": { my_target.innerHTML += "必须是数字"; } break;
            case "spn_jianjie": { my_target.innerHTML += "请填写,字数至少50个"; } break;
            case "spn_kehu": { my_target.innerHTML += "请填写服务过的客户,如无留空"; } break;
            case "spn_zhujiang": { my_target.innerHTML += "请填写,如无留空"; } break;
            case "td_pic": { my_target.innerHTML += "请上传公司logo(规格:130*50 图片格式:gif,jpg)"; } break;
    ...
    function fnt_uid(ajax) {
        ...
        if (reg_uid.value == "") { td_uid.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>必填</font>"; return "请填写用户名"; }
            ...
            else { td_uid.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>用户名错误 字母,数字 4～20字符</font>"; return "用户名错误 字母,数字 6～20字符"; }
        }
    }
    function ajax_uid() {
        var reg_uid = document.getElementById("reg_uid");
    ...
    function fnt_pwd1() {
        ...
        if (reg_pwd1.value == "") { td_pwd1.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>必填</font>"; return "请填写密码"; }
            ...
            else { td_pwd1.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>密码错误 字母,数字 6～20字符</font>"; return "密码错误 字母,数字 6～20字符"; }
        }
    }
    //确认密码
    function fnt_pwd2() {
    ...
    function fnt_phone() {
        ...
        if (reg_phone1.value == "") { message += ",区号必填"; }
        else { if (rg_phone1.test(reg_phone1.value) == false) { message += ",区号错误(应是3～4位数字)"; } }

        if (reg_phone2.value == "") { message += ",电话号必填"; }
        else { if (rg_phone2.test(reg_phone2.value) == false) { message += ",电话号错误(应是7～8位数字)"; } }

        if (rg_phone3.test(reg_phone3.value) == false) { message += ",分机号错误(应是6位数字以内)"; }

        if (message == "固定电话") { td_phone.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/ok.gif' />"; return ""; }
    ...
    function fnt_mobile() {
        ...
        if (reg_mobile.value == "") { td_mobile.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>必填</font>"; return "请填移动电话"; }
        else {
            if (rg_mobile.test(reg_mobile.value) == true) { td_mobile.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/ok.gif' />"; return ""; }
    ...
    //讲师姓名
    function fnt_xingming() {
        var reg_xingming = document.getElementById("reg_xingming");
        var td_xingming = document.getElementById("td_xingming");
        if (reg_xingming.value == "") { td_xingming.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>必填</font>"; return "请填讲师姓名"; }
        else { td_xingming.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/ok.gif' />"; return ""; }
    }
    //省市
    ...
    //对外课酬
    function fnt_pay() {
        var reg_pay = document.getElementById("reg_pay");
        var td_pay = document.getElementById("td_pay");
        var rg_pay = /^\d{1,6}$/
        if (reg_pay.value == "") { td_pay.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>必填</font>"; return "请填写对外课酬"; }
        else {
            if (rg_pay.test(reg_pay.value) == true) { td_pay.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/ok.gif' />"; return ""; }
            else { td_pay.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>对外课酬必须是数字</font>"; return "对外课酬必须是数字"; }
        }
    }
    //领域
    function fnt_lingyu() {
        var my_count = 0;
        for (i = 0; i <= 13; i++) {
            var my_lingyu = document.getElementById("reg_lingyu_" + i);
            if (my_lingyu.checked == true) { my_count += 1; }
        }
        var spn_lingyu = document.getElementById("spn_lingyu");
        if (my_count == 0) { spn_lingyu.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>请填培训领域</font>"; return "请填培训领域"; }
        else if (my_count <= 2) { spn_lingyu.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/ok.gif' />"; return ""; }
        else { spn_lingyu.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>最多选2项</font>"; return "培训领域最多选2项"; }
    }
    //公司简介
    function fnt_jianjie_count() {
        var reg_jianjie = document.getElementById("reg_jianjie");
        var jianjie_count = document.getElementById("jianjie_count");
        if (reg_jianjie.value.length < 50 | reg_jianjie.value.length > 5000) { jianjie_count.innerHTML = "(<font color='red'>" + reg_jianjie.value.length + "</font>)"; }
        else { jianjie_count.innerHTML = "(<font color='green'>" + reg_jianjie.value.length + "</font>)"; }
    }
    function fnt_jianjie() {
        var reg_jianjie = document.getElementById("reg_jianjie");
        var spn_jianjie = document.getElementById("spn_jianjie");
        if (reg_jianjie.value.length < 50 | reg_jianjie.value.length > 5000) { spn_jianjie.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>字数至少50字</font>"; return "字数至少50字"; }
        else { spn_jianjie.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/ok.gif' />"; return ""; }

    }
    //服务过的客户
    function fnt_kehu() {
        var spn_kehu = document.getElementById("spn_kehu");
        spn_kehu.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/ok.gif' />";
        return "";
    }
    //主讲
    function fnt_zhujiang() {
        var spn_zhujiang = document.getElementById("spn_zhujiang");
        spn_zhujiang.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/ok.gif' />";
        return "";
    }
   
    //logo
    function fnt_pic() {
        var file_pic = document.getElementById("<%=fudPhoto.ClientID %>");
        var td_pic = document.getElementById("td_pic");
        var li_name = file_pic.value.split(".");
        if (file_pic.value == "" | li_name[li_name.length - 1].toLowerCase() == "gif" | li_name[li_name.length - 1].toLowerCase() == "jpg") { td_pic.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/ok.gif' />"; return ""; }
        else { td_pic.innerHTML = "<img src='<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/error.gif' /><font color='red'>图片格式不正确</font>"; return "图片格式不正确"; }
    }
    ...
    function fnt_sbm(oEvent) {
        ...
        if (err_message == "") { err_message = fnt_xingming(); err_target = "reg_xingming"; } else { fnt_xingming(); }
        if (err_message == "") { err_message = fnt_sheng_shi(); err_target = "reg_sheng"; } else { fnt_sheng_shi(); }
        if (err_message == "") { err_message = fnt_pay(); err_target = "reg_pay"; } else { fnt_pay(); }
        if (err_message == "") { err_message = fnt_lingyu(); err_target = "reg_lingyu"; } else { fnt_lingyu(); }
        if (err_message == "") { err_message = fnt_jianjie(); err_target = "reg_jianjie"; } else { fnt_jianjie(); }
        if (err_message == "") { err_message = fnt_kehu(); err_target = "reg_kehu"; } else { fnt_kehu(); }
        if (err_message == "") { err_message = fnt_zhujiang(); err_target = "reg_zhujiang"; } else { fnt_zhujiang(); }
        if (err_message == "") { err_message = fnt_pic(); err_target = "file_pic"; } else { fnt_pic(); }
...
<tr class="head"><td colspan="3" style="background:url(<%=Utils.AbsoluteWebRoot %>themes/Defualt/pic/btn6.jpg) left bottom no-repeat;">讲师详细信息</td></tr>

<tr><td class="td1"><span>*</span> 讲师姓名：</td><td><input name="reg_xingming" id="reg_xingming" type="text" maxlength="50" onblur="fnt_xingming()" onfocus="fnt_on('td_xingming')" /></td><td id="td_xingming" class="td3">讲师姓名</td></tr>

<tr><td class="td1"><span>*</span> 固定电话：</td><td><input name="reg_phone1" id="reg_phone1" type="text" maxlength="4" onblur="fnt_phone()" onfocus="fnt_on('td_phone')" /> - <input name="reg_phone2" id="reg_phone2" type="text" maxlength="8" onblur="fnt_phone()" onfocus="fnt_on('td_phone')" /> - <input name="reg_phone3" id="reg_phone3" type="text" maxlength="6" onblur="fnt_phone()" onfocus="fnt_on('td_phone')" /></td><td id="td_phone" class="td3">请正确填写,如(020-12345678-888),如无分机请留空</td></tr>

<tr><td class="td1"><span>*</span> 移动电话：</td><td><input name="reg_mobile" id="reg_mobile" type="text" maxlength="11" onblur="fnt_mobile()" onfocus="fnt_on('td_mobile')" /></td><td id="td_mobile" class="td3">请正确填写11位手机号码</td></tr>
...
<tr><td class="td1"><span>*</span> 对外课酬：</td><td><input name="reg_pay" id="reg_pay" type="text" maxlength="6" onblur="fnt_pay()" onfocus="fnt_on('td_pay')" /><span class="spn_pay">元/天</span></td><td id="td_pay" class="td3">必须是数字</td></tr>



<tr><td class="td1"><span>*</span> 培训领域：</td><td colspan="2">
<asp:PlaceHolder runat="server" id="phFields"></asp:PlaceHolder>          
<span id="spn_lingyu">最多2项</span></td></tr>



<tr><td class="td1"><span>*</span> 讲师简介：</td><td colspan="2"><textarea name="reg_jianjie" id="reg_jianjie" onkeyup="fnt_jianjie_count()" onfocus="fnt_on('spn_jianjie')" onblur="fnt_jianjie()"></textarea><span id="jianjie_count"></span><span id="spn_jianjie">请填写,字数至少50个</span></td></tr>

<tr><td class="td1">曾服务客户：</td><td colspan="2"><textarea name="reg_kehu" id="reg_kehu" onfocus="fnt_on('spn_kehu')" onblur="fnt_kehu()"></textarea><span id="spn_kehu">请填写服务过的客户,如无留空</span></td></tr>

<tr><td class="td1">主讲课程：</td><td colspan="2"><textarea name="reg_zhujiang" id="reg_zhujiang" onfocus="fnt_on('spn_zhujiang')" onblur="fnt_zhujiang()"></textarea><span id="spn_zhujiang">请填写,如无留空</span></td></tr>

<tr><td class="td1">讲师照片：</td><td><asp:FileUpload ID="fudPhoto" runat="server" /></td><td id="td_pic" class="td3">请上传个人照片(规格:100*120 图片格式:gif,jpg)</td></tr>
